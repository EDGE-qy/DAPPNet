...
def push_prototypes(dataloader, # pytorch dataloader (must be unnormalized in [0,1])
    ...
    for push_iter, (search_batch_input, search_y, index) in enumerate(dataloader):
        '''
    ...
    #     print('prototype index for each node: %d'%proto_bound_boxes[j, 0])
    
    # prototype_network_parallel.module.image_index_of_prototypes = proto_bound_boxes[:, 0]
    # if isinstance(prototype_network_parallel, torch.nn.DataParallel):
    #     for device_id in prototype_network_parallel.device_ids:
    #         getattr(prototype_network_parallel.module, 'image_index_of_prototypes').data = \
    #             getattr(prototype_network_parallel.module, 'image_index_of_prototypes').data.copy_(proto_bound_boxes[:, 0].cuda())
    ...
    # print(prototype_network_parallel.module.image_index_of_prototypes.shape)
...
def update_prototypes_on_batch(search_batch_input,
    ...
    for j in range(n_prototypes):
        ...
        if batch_max_proto_act_j > global_max_proto_act[j]:
            ...
            print('data for %d th: %d, %d, %d'%(j, proto_bound_boxes[j, 0], batch_argmax_proto_act_j[0], start_index_of_search_batch))

...
def isin(a, b):
    return (a.unsqueeze(1) == b).any(1)


def _train_or_test(model, dataloader, optimizer=None, class_specific=True, use_l1_mask=True,
                   coefs=None, log=print, subtractive_margin=True, use_ortho_loss=False, self_mask=False,
                   l1=False):
    '''
    ...
    for i, (image, label, index) in enumerate(dataloader):
        input = image.cuda()
        target = label.cuda()
        i_can_see_myself = None
        ...
        # if False:
            ...
            if len(i_can_see_myself) > 0:
                # print('i_can_see_myself:', i_can_see_myself)
        ...
        with grad_req:
            ...
            if subtractive_margin:
                output, additional_returns = model(input, is_train=is_train, 
                                                    prototypes_of_wrong_class=prototypes_of_wrong_class, index_of_the_batch=i_can_see_myself)
            else:
                output, additional_returns = model(input, is_train=is_train, prototypes_of_wrong_class=None, index_of_the_batch=i_can_see_myself)
        ...
        if is_train:
            wandb.log({'train/max_offset': max_offset.item(), 'train/l2': offset_l2.item(),
                    'train/cluster_cost': cluster_cost.item(), 'train/separation_cost': separation_cost.item(),
                    'train/avg_separation_cost': avg_separation_cost.item(), 'train/l1': l1.item(),
                    'train/cross_entropy': cross_entropy.item(), 'train/orthogonality_loss': orthogonality_loss.item(),
        ...
        if is_train and self_mask and len(i_can_see_myself) > 0:
        # if False:
            model.module.prototype_vectors.data[i_can_see_myself] = original_prototypes
            del original_prototypes
...
def train(model, dataloader, optimizer, class_specific=False, coefs=None, 
            log=print, subtractive_margin=True, use_ortho_loss=False, self_mask=False, l1=False):
    ...
    return _train_or_test(model=model, dataloader=dataloader, optimizer=optimizer,
                          ...
                          subtractive_margin=subtractive_margin, use_ortho_loss=use_ortho_loss, self_mask=self_mask, l1=l1)


def test(model, dataloader, class_specific=False, log=print, subtractive_margin=True, self_mask=False,
         l1=False):
    log('\ttest')
    model.eval()
    return _train_or_test(model=model, dataloader=dataloader, optimizer=None,
                          class_specific=class_specific, log=log, subtractive_margin=subtractive_margin,
                          self_mask=self_mask, l1=l1)

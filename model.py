...
class PPNet(nn.Module):
    ...
    def __init__(self, features, img_size, prototype_shape, 
                 ...
                 num_offset_head=1, dropout=0.0, clip_offset=False):
        ...
        # TODO: Update this. Trainloader. Push. 已经完成在push之后把所有的prototype的index都存起来了。
        ...
        self.topk_k = topk_k
        self.num_offset_head = num_offset_head
        ...
        self.prototype_class_identity = torch.zeros(self.num_prototypes,
                                                    self.num_classes)
        # print(self.prototype_class_identity.shape, self.num_prototypes, self.num_classes)
    ...
    def cos_activation(self, x, is_train=True, 
                        prototypes_of_wrong_class=None, is_push=False, index_of_the_batch=None):
        # TODO: pass in the parameter index_of_the_batch
        '''
        ...
        if is_train and not is_push and index_of_the_batch is not None and len(index_of_the_batch) > 0:
            ...
            normalized_prototypes = normalized_prototypes * factor  # TODO: implement this multiple on different dims
            # print("Prototype has been multiplied by factor: ", factor.shape)
            

        # Compute offsets for this input
        offset = self.conv_offset(x_normalized)
        # here the num_channel is offset * num_heads
        ...
        if self.using_deform:
            ...
            if self.num_offset_head == 1:
                activations_dot = NormPreserveDeformConvFunction.apply(x_normalized, offset, 
                                                                    ...
                                                                    1, #im2col_step
                                                                    True) #zero_padding
                # print('activations_dot:', activations_dot.shape)
            else:
                ...
                for ofst in offsets:
                    activ_dot = NormPreserveDeformConvFunction.apply(x_normalized, ofst, 
                                                                ...
                                                                1, #im2col_step
                                                                True) #zero_padding
                    # print('activ_dot:', activ_dot.shape)
        ...
        # if self.clip_offset:
        #     offset = torch.clamp(offset, -10, 10)
            
        return activations, marginless_activations

    def prototype_activations(self, x, is_train=True, prototypes_of_wrong_class=None, index_of_the_batch=None):
        '''
        x is the raw input
        '''
        conv_features = self.conv_features(x)
        activations, marginless_activations = self.cos_activation(conv_features, is_train=is_train, 
                                                        prototypes_of_wrong_class=prototypes_of_wrong_class, index_of_the_batch=index_of_the_batch)
        return activations, [marginless_activations, conv_features]

    def forward(self, x, is_train=True, prototypes_of_wrong_class=None, index_of_the_batch=None):
        activations, additional_returns = self.prototype_activations(x, is_train=is_train, 
                                                prototypes_of_wrong_class=prototypes_of_wrong_class, index_of_the_batch=index_of_the_batch)
        print('activations:', activations[0][0][0][0])
        ...
        # drop_activations = F.dropout(topk_activations, p=self.dropout, training=is_train)
        drop_activations = topk_activations
        mean_activations = torch.mean(drop_activations, dim=-1)
    ...
    def push_forward(self, x):
        ...
        _, marginless_activations = self.cos_activation(conv_output, is_push=True)
...
def construct_PPNet(base_architecture, pretrained=True, img_size=224,
    ...
    return PPNet(features=features,
                 ...
                 prototype_dilation=prototype_dilation, num_offset_head=num_offset_head,
